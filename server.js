...
if (!PAY_TO) { console.error("Set WALLET_ADDRESS in .env"); process.exit(1); }
...
app.use(paymentMiddleware(
  {
    "GET /api/weather":  { accepts: { scheme:"exact", price:"$0.003", network:NETWORK, payTo:PAY_TO }, description:"Current weather", mimeType:"application/json" },
    "GET /api/stocks":   { accepts: { scheme:"exact", price:"$0.005", network:NETWORK, payTo:PAY_TO }, description:"Stock quote",     mimeType:"application/json" },
    "GET /api/crypto":   { accepts: { scheme:"exact", price:"$0.002", network:NETWORK, payTo:PAY_TO }, description:"Crypto price",    mimeType:"application/json" },
    "GET /api/research": { accepts: { scheme:"exact", price:"$0.025", network:NETWORK, payTo:PAY_TO }, description:"AI research",     mimeType:"application/json" },
  },
  resourceServer,
));
...
app.get("/api/weather", async (req, res) => {
  ...
  try {
    ...
    const wx = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,relative_humidity_2m,wind_speed_10m,weather_code&temperature_unit=fahrenheit&wind_speed_unit=mph`).then(r=>r.json());
...
app.get("/api/research", async (req, res) => {
  ...
  try {
    const msg = await ai.messages.create({ model:"claude-sonnet-4-20250514", max_tokens:300, system:"You are a concise factual research API. Answer in 2-4 sentences.", messages:[{role:"user",content:question}] });
    record("/api/research", 0.025, question.slice(0,60));
    res.json({ question, answer:msg.content[0].text });
  } catch(err) { res.status(500).json({ error:err.message }); }
});

app.get("/", (req, res) => res.json({ name:"x402.market", network:NETWORK, endpoints:[
...
app.get("/earnings", (req, res) => res.json({ total_usdc:earnings.total.toFixed(6), calls:earnings.calls, recent:earnings.log.slice(0,20) }));
